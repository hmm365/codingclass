<?php
    ...
    $sql = "SELECT * FROM myBlog WHERE blogID = $myBlogID";
                    ...
                    <form action="blogModifySave.php" name="blogWrite" method="post" enctype="multipart/form-data">
                        <fieldset>
                            ...
                            <div>
                                ...
                                <input type="hidden" name="blogId" id="blogId" value='<?=$myBlogID?>'>
                            </div>
                            <div>
                                <label for="blogTitle">제목</label>
                                <input type="text" name="blogTitle" id="blogTitle" value='<?=$content = str_replace("<br />","\n",$info['blogTitle'])?>'>
                            </div>
                            <div>
                                <label for="blogContents">내용</label>
                                <textarea name="blogContents" id="blogContents" rows="20"><?=$content = str_replace("<br />","\n",$info['blogContents'])?></textarea>
                            </div>
                            <div>
                                <label for="blogImgFile">파일</label>
                                <input type="file" accept="image/jpeg, image/png, image/jpg, image/png, image/gif" name="blogImgFile" id="blogImgFile" value='<?=$info['blogImgSrc']?>'>
                            </div>
